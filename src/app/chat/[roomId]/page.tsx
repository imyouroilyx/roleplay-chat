...
import { Message, MemberData } from '@/types/chat';
...
export default function ChatPage({ params }: { params: Promise<{ roomId: string }> }) {
  ...
  useEffect(() => {
    ...
    channel.bind('pusher:subscription_succeeded', (m: any) => {
      ...
      m.each((member: any) => {
        list.push({ id: member.id, name: member.info.name, isAdmin: member.info.isAdmin });
    ...
    channel.bind('pusher:member_added', (member: any) => {
      ...
      setMembers(prev => [...prev, { id: member.id, name: member.info.name, isAdmin: member.info.isAdmin }]);
    });
  ...
  const handleChangeName = async () => {
    if (!newName.trim() || newName === nickname) {
    ...
    await fetch('/api/chat/send', {
      ...
      body: JSON.stringify({
        type: 'system',
        text: `>>> 🔄 ${oldName} ทำการเปลี่ยนชื่อดิสเพลย์เป็น ${newName}`,
        roomId: safeRoomId,
        timestamp: new Date().toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' }),
        isNameChange: true, 
        oldName: oldName,
  ...
  const saveRoom = async () => {
    ...
    const adminPass = localStorage.getItem('rp_admin_key');
    await fetch('/api/admin/rooms', {
      method: 'POST',
      body: JSON.stringify({ action: 'save', roomName: newRoom.name, type: newRoom.type, roomPassword: newRoom.pass, adminPassword: adminPass })
    });
  ...
  const handleSwitchRoom = async () => {
    ...
    if (switchTargetRoom.type === 'private') {
      const res = await fetch('/api/rooms/verify', {
        method: 'POST',
        body: JSON.stringify({ roomName: switchTargetRoom.name, password: switchPass })
      });
                  ...
                  {isMe ? (
                    <>
                      <span 
                        ...
                        className={`font-black cursor-pointer transition-all ${m.isAdmin ? "bg-gradient-to-r from-blue-400 via-white to-blue-500 bg-[length:200%_auto] animate-gradient-x bg-clip-text text-transparent" : "text-white underline decoration-neutral-800"}`}
                        style={m.isAdmin ? { filter: 'drop-shadow(0 0 12px rgba(59, 130, 246, 0.95))' } : {}}
                      >
                        ● {m.name} {openUserMenu ? '▴' : '▾'}
                      </span>
                      <AnimatePresence>
                        {openUserMenu && (
                          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden pl-4 mt-2">
                            <button onClick={() => setIsChangingName(true)} className="text-[10px] bg-neutral-900 text-neutral-400 hover:text-white px-3 py-2 rounded uppercase border border-neutral-800 transition-all w-fit">
                              แก้ไขชื่อดิสเพลย์
                            </button>
                          </motion.div>
                        )}
                    ...
                    <span 
                      onClick={() => setRecipient(m.name)} 
                      className={`font-black cursor-pointer transition-all ${m.isAdmin ? "bg-gradient-to-r from-blue-400 via-white to-blue-500 bg-[length:200%_auto] animate-gradient-x bg-clip-text text-transparent" : "text-neutral-600 hover:text-neutral-400"}`}
                      style={m.isAdmin ? { filter: 'drop-shadow(0 0 12px rgba(59, 130, 246, 0.95))' } : {}}
                    >
                      ○ {m.name}
                    </span>
              ...
              <div className="bg-neutral-950 border border-neutral-900 p-8 rounded-2xl w-full max-w-sm space-y-4 shadow-2xl">
                ...
                <div className="space-y-2 max-h-56 overflow-y-auto custom-scrollbar">
                  {allRooms.map(r => (
                    <div 
                      key={r.name} 
                      onClick={() => { setSwitchTargetRoom(r); setSwitchPass(''); }} 
                      className={`p-3 border rounded-lg cursor-pointer transition-all ${switchTargetRoom?.name === r.name ? 'border-blue-500 bg-blue-900/20' : 'border-neutral-800 bg-neutral-900/30 hover:bg-neutral-800'}`}
                    >
                ...
                <AnimatePresence>
                  {switchTargetRoom?.type === 'private' && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                      <input 
                        type="password" placeholder="รหัสผ่านห้องส่วนตัว..." 
                        value={switchPass} onChange={(e) => setSwitchPass(e.target.value)} 
                        className="w-full bg-black border border-red-900/50 p-3 mt-2 text-sm outline-none focus:border-red-500 transition-all rounded text-white text-center" 
                      />
                    </motion.div>
                  )}
                </AnimatePresence>
        ...
        <AnimatePresence>
          {showAdminPanel && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/95 z-50 flex items-center justify-center p-6 backdrop-blur-md">
              <div className="bg-neutral-950 border border-neutral-900 p-8 rounded-3xl w-full max-w-2xl max-h-[80vh] flex flex-col shadow-2xl overflow-hidden">
                <h3 className="text-sm font-black text-blue-500 tracking-[0.4em] uppercase mb-6 text-center">จัดการระบบห้อง</h3>
                  ...
                  <div className="flex-1 flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar text-xs">
                      {Object.entries(roomList).map(([name, meta]: [string, any]) => {
                        let m;
                        try { m = typeof meta === 'string' ? JSON.parse(meta) : meta; } 
                        catch (e) { m = { type: 'public', roomPassword: '' }; }
                        return (
                          <div key={name} className="flex items-center justify-between p-3 border border-neutral-900 rounded bg-neutral-900/30">
                            ...
                            <div className="flex gap-3">
                              ...
                              <button onClick={async () => { if(confirm(`ลบห้อง ${name}?`)) { await fetch('/api/admin/rooms', {method:'POST', body: JSON.stringify({action:'delete', roomName:name, adminPassword:localStorage.getItem('rp_admin_key')})}); fetchRoomsAdmin(); } }} className="text-red-500 font-bold text-[10px] hover:underline">ลบ</button>
                            </div>
                          </div>
                        );
                      })}
          ...
          <div className="max-w-5xl mx-auto space-y-4">
            ...
            <div className="flex gap-3 border-b border-neutral-800 focus-within:border-white transition-all">
              <input 
                ...
                placeholder={recipient === 'all' ? "ป้อนคำสั่งหรือข้อความ..." : `กำลังกระซิบหา ${recipient}...`}
